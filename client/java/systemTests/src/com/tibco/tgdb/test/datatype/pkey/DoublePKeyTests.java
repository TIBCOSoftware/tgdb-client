...
public class DoublePKeyTests {
  ...
  /**
	...
	public void testDefinePKey() throws Exception {
		...
		TGNodeType nodeAllAttrsType = gmd.getNodeType("nodeDoubleKey");
		...
		for (int i=0; i<data.length; i++) {
			...
			//node.setAttribute("doubleAttr", i);
			nodes.add(node);
		...
		//edge.setAttribute("doubleAttr2", booleanData[booleanData.length-1][0]);
		//conn.insertEntity(edge);
		conn.commit();
	...
	public void testRetrievePKey() throws Exception {
		...
		TGKey tgKey = gof.createCompositeKey("nodeDoubleKey");
		...
		for (int i=0; i<data.length; i++) {
			...
			//System.out.println("READ ATTR:" + entity.getAttribute("doubleAttr").getAsString());
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
	...
	public void testRetrieveUpdatedPKey() throws Exception {
		...
		for (int i=0; i<data.length; i++) {
			...
			//System.out.println("READ UPDATED ATTR:" + entity.getAttribute("doubleAttr").getAsString());
			// Assert on Node attribute
			Assert.assertFalse(entity.getAttribute("doubleAttr").isNull(), "Expected attribute #"+i+" non null but found it null -");
			Assert.assertEquals(entity.getAttribute("doubleAttr").getAsDouble(), data[i][1]);
		}
		conn.disconnect();
	}
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
		...
		for (int i=0; i<data.length; i++) {
			...
			entity.setAttribute("doubleAttr", null); // delete the string value by setting it up to null
			conn.updateEntity(entity);
			conn.commit();
		}
		conn.disconnect();
