...
public class DatePKeyTests {
  ...
  /**
	...
	public void testDefinePKey() throws Exception {
		...
		TGNodeType nodeAllAttrsType = gmd.getNodeType("nodeDateKey");
	...
	public void testRetrievePKey() throws Exception {
		...
		TGKey tgKey = gof.createCompositeKey("nodeDateKey");
		
		Object[][] data = this.getDateData();
		for (int i=0; i<data.length; i++) {
			tgKey.setAttribute("dateAttr", data[i][0]);
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
		...
		for (int i=0; i<data.length; i++) {
			...
			entity.setAttribute("dateAttr", null); // delete the string value by setting it up to null
			conn.updateEntity(entity);
			conn.commit();
		}
		conn.disconnect();
