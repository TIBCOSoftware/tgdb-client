...
/**
 * CRUD tests for char data type primary key
 */
public class CharPKeyTests {
  ...
  /**
	 * testCreateCharData - Insert nodes with char pkey
	 * @throws Exception
	 */
	@Test(description = "Insert nodes with char pkey")
	public void testDefinePKey() throws Exception {
		...
		TGNodeType nodeAllAttrsType = gmd.getNodeType("nodeCharKey");
		...
		//edge.setAttribute("charAttr2", booleanData[booleanData.length-1][0]);
		//conn.insertEntity(edge);
		conn.commit();
	...
	/**
	 * testReadCharData - Retrieve nodes with char pkey
	 * @throws Exception
	 */
	
	@Test(description = "Retrieve nodes with char pkey",
	...
	public void testRetrievePKey() throws Exception {
		...
		TGKey tgKey = gof.createCompositeKey("nodeCharKey");
	...
	/**
	 * testUpdateCharData - Update char attribute
	 * @throws Exception
	 */
	
	@Test(description = "Update char attribute",
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
	...
	public void testRetrieveUpdatedPKey() throws Exception {
		...
		for (int i=0; i<data.length; i++) {
			...
			Assert.assertEquals(entity.getAttribute("charAttr").getAsChar(), data[i][1]);
		}
		conn.disconnect();
	}
	
	/**
	 * testDeleteCharData - Delete char attribute
	 * @throws Exception
	 */
	
	@Test(description = "Delete char attribute",
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
	...
	/**
	 * testReadDeletedCharData - Retrieve nodes with updated char attribute
	 * @throws Exception
	 */
	
	@Test(description = "Retrieve nodes with deleted char attribute",
	...
	public Object[][] getCharData() throws IOException, EvalError {
		Object[][] data =  PipedData.read(this.getClass().getResourceAsStream("/"+this.getClass().getPackage().getName().replace('.', '/') + "/char.data"));
		return data;
	}
}
