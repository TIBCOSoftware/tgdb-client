...
public class ShortPKeyTests {
  ...
  /**
	...
	public void testDefinePKey() throws Exception {
		...
		TGNodeType nodeAllAttrsType = gmd.getNodeType("nodeShortKey");
	...
	public void testRetrievePKey() throws Exception {
		...
		TGKey tgKey = gof.createCompositeKey("nodeShortKey");
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
	...
	public void testRetrieveUpdatedPKey() throws Exception {
		...
		for (int i=0; i<data.length; i++) {
			...
			Assert.assertEquals(entity.getAttribute("shortAttr").getAsShort(), data[i][1]);
		}
		conn.disconnect();
	}
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
		...
		for (int i=0; i<data.length; i++) {
			...
			entity.setAttribute("shortAttr", null); // delete the string value by setting it up to null
			conn.updateEntity(entity);
			conn.commit();
		}
		conn.disconnect();
