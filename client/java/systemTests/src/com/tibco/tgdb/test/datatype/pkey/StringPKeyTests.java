...
/**
 * CRUD tests for string data type primary key
 */
public class StringPKeyTests {
  ...
  /**
	 * testCreateBooleanData - Insert nodes  with string pkey
	 * @throws Exception
	 */
	@Test(description = "Insert nodes with string pkey")
	public void testDefinePKey() throws Exception {
		...
		TGNodeType nodeAllAttrsType = gmd.getNodeType("nodeStringKey");
	...
	public void testRetrievePKey() throws Exception {
		...
		TGKey tgKey = gof.createCompositeKey("nodeStringKey");
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
	...
	public void testRetrieveUpdatedPKey() throws Exception {
		...
		for (int i=0; i<data.length; i++) {
			...
			Assert.assertEquals(entity.getAttribute("stringAttr").getAsString(), data[i][1]);
		}
		conn.disconnect();
	}
...
TGConnection conn = TGConnectionFactory.getInstance().createConnection(tgUrl, tgUser, tgPwd, null);
	...
	public Object[][] getStringData() throws IOException, EvalError {
		Object[][] data =  PipedData.read(this.getClass().getResourceAsStream("/"+this.getClass().getPackage().getName().replace('.', '/') + "/string.data"));
		return data;
	}
