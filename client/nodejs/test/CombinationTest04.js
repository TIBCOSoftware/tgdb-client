...
function fetchMetadata(conn, callback) {
		...
		if (attrDesc !== null) {
			...
			logger.logInfo("'factor' is not found from meta data fetch");
		}
		...
		if (attrDesc !== null) {
			...
			logger.logInfo("'level' is not found from meta data fetch");
		}
...
function createCommit(conn, callback) {
    ...
    node1.setAttribute("nickname", "ç¾Žéº—");
    ...
    //node3.setAttribute("æ˜¯è¶…äººå—Ž", false);
