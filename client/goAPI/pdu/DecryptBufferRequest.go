 ...
 *
 * File name: DecryptBufferRequest.go
...
func (msg *DecryptBufferRequestMessage) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning DecryptBufferRequestMessage::FromBytes resulted in '%+v'", msg))
	return msg, nil
}
...
func (msg *DecryptBufferRequestMessage) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning DecryptBufferRequestMessage::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
