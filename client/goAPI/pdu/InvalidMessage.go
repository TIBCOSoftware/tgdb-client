...
func (msg *InvalidMessage) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning InvalidMessage::FromBytes resulted in '%+v'", msg))
	return msg, nil
}
...
func (msg *InvalidMessage) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning InvalidMessage::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
