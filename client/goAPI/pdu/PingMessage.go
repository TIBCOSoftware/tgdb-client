...
func (msg *PingMessage) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning PingMessage::FromBytes resulted in '%+v'", msg))
	return msg, nil
}
...
func (msg *PingMessage) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning PingMessage::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
