...
func (msg *MetadataRequest) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning MetadataRequest::FromBytes resulted in '%+v'", msg))
	return msg, nil
}
...
func (msg *MetadataRequest) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning MetadataRequest::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
