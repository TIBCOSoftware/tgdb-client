 ...
 *
 * File name: DecryptBufferResponse.go
...
func (msg *DecryptBufferResponseMessage) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning DecryptBufferResponseMessage::FromBytes resulted in '%+v'", msg))
	return msg, nil
}
...
func (msg *DecryptBufferResponseMessage) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning DecryptBufferResponseMessage::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
