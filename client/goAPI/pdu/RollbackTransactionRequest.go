 ...
 *
 * File name: RollbackTransactionRequest.go
...
func (msg *RollbackTransactionRequestMessage) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning RollbackTransactionRequestMessage::FromBytes resulted in '%+v'", msg))
	return msg, nil
}
...
func (msg *RollbackTransactionRequestMessage) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning RollbackTransactionRequestMessage::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
