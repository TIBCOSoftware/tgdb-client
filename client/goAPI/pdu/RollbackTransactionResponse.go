 ...
 *
 * File name: RollbackTransactionResponse.go
...
func (msg *RollbackTransactionResponseMessage) FromBytes(buffer []byte) (types.TGMessage, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning RollbackTransactionResponseMessage::FromBytes results in '%+v'", msg))
	return msg, nil
}
...
func (msg *RollbackTransactionResponseMessage) ToBytes() ([]byte, int, types.TGError) {
	...
	logger.Log(fmt.Sprintf("Returning RollbackTransactionResponseMessage::ToBytes results bytes-on-the-wire in '%+v'", os.GetBuffer()))
