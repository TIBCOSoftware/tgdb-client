 ...
 * Removed  provided implementations, and made it as a pure interface as James Gosling intended to.
 */
public interface VertexProgram<M> extends Cloneable {

    static final String VERTEX_PROGRAM = "gremlin.vertexProgram";
     ...
     * The  implementation provided simply stores the VertexProgram class name for reflective reconstruction.
     ...
     */
    void execute(final Vertex vertex, final Messenger<M> messenger, final Memory memory);
     ...
     */
    void workerIterationStart(final Memory memory);
     ...
     */
    void workerIterationEnd(final Memory memory);
    /**
     ...
     */
    Optional<MessageCombiner<M>> getMessageCombiner();
     ...
     */
    Set<MessageScope> getMessageScopes(final Memory memory);
     ...
     * The  implementation simply returns the object as it assumes that the VertexProgram instance is a stateless singleton.
     *
     ...
     */
    static <V extends VertexProgram> V createVertexProgram(final Graph graph, final Configuration configuration) {
        try {
    ...
    interface Builder {
         ...
         */
        Builder configure(final Object... keyValues);

        <P extends VertexProgram> P create(final Graph graph);
    ...
    interface Features {
        boolean requiresGlobalMessageScopes();

        boolean requiresLocalMessageScopes();

        boolean requiresVertexAddition();

        boolean requiresVertexRemoval();

        boolean requiresVertexPropertyAddition();

        boolean requiresVertexPropertyRemoval();

        boolean requiresEdgeAddition();

        boolean requiresEdgeRemoval();

        boolean requiresEdgePropertyAddition();

        boolean requiresEdgePropertyRemoval();
    }
}
