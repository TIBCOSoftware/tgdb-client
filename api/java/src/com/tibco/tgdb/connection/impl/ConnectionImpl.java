 ...
 * Created by: suresh 
 * <p/>
 * SVN Id: $Id: ConnectionImpl.java 4426 2020-09-29 00:21:22Z kattaylo $
 */
...
import com.tibco.tgdb.model.TGChangeListener;
...
public class ConnectionImpl implements TGConnection, TGChangeListener {
    ...
    private void sendConnectionProperties() throws TGException
    {
        ConnectionPropertiesMessage request = (ConnectionPropertiesMessage) TGMessageFactory.getInstance().
                createMessage(VerbId.ConnectionPropertiesMessage, channel.getAuthToken(), channel.getSessionId());
    ...
    public TGResultSet commit() throws TGException {
        ...
        if ((addedList.isEmpty()) && (changedList.isEmpty()) && (removedList.isEmpty())) {
            //Nothing to Commit. TGDB-807 issue.
            return null;
        }
...
//            request.setConnectionId(connId);
            ...
            if (response.hasException()) {
                TGException exception = response.getException();
...
//            request.setConnectionId(connId);
    ...
    public TGResultSet<Object> executeQuery(String query, TGQueryOption queryOption) throws TGException {
        ...
        if (idx != -1) {
			...
			if (prefix.equalsIgnoreCase("tgql")) {
				...
				return executeGremlinQuery(query.substring(idx + 3), command.EXECUTEGREMLINSTR.getValue(), queryOption);
            } else if (prefix.equalsIgnoreCase("gbc")) {
                return executeGremlinQuery(query.substring(idx + 3), command.EXECUTEGREMLINBYTECODE.getValue(), queryOption);
			} else {
        	...
        	if (lang.equalsIgnoreCase("tgql")) {
        		...
        		return executeGremlinQuery(query, command.EXECUTEGREMLINSTR.getValue(), queryOption);
            } else if (lang.equalsIgnoreCase("gbc")) {
                return executeGremlinQuery(query, command.EXECUTEGREMLINBYTECODE.getValue(), queryOption);
        	} else {
        		return new ObjectResultSet(this, 0);
        	}
        }
	...
	public TGResultSet<Object> executeGremlinQuery(String query, int queryCmd, TGQueryOption queryOption) throws TGException {
    	initMetadata();
        connPool.adminlock();
        		...
        		TGEntity.TGEntityKind kind = TGEntity.TGEntityKind.fromValue(entityStream.readByte());
          		if (kind != TGEntity.TGEntityKind.InvalidKind) {
        	        ...
        	        if (gLogger.isEnabled(TGLogger.TGLevel.Debug)) {
           			...
           			gLogger.log(TGLevel.Warning, "executeQuery - Received invalid entity kind %s", kind.name());
           		}
        	}
            return resultSet;
        } catch (IOException ioe) {
