 ...
 * Created by: vincent
 * SVN Id: $Id: GremlinResult.java 4492 2020-10-16 18:34:13Z dhudson $
 */
...
public class GremlinResult {
    ...
    static public void constructList(TGInputStream entityStream, GraphObjectFactoryImpl gof, Collection col) throws IOException, TGException {
   		...
   		for (int i=0; i<size; i++) {
   			if (elemType == ElementType.Entity) {
   				...
   				col.add(colElem);
			} else if (elemType == ElementType.MixedList) {
				Collection colElem = new ArrayList();
				constructMixedList(entityStream, gof, colElem);
				col.add(colElem);
	...
	static public void constructMixedList(TGInputStream entityStream, GraphObjectFactoryImpl gof, Collection col) throws IOException, TGException {
		int size = entityStream.readInt();
		for (int i=0; i<size; i++) {
			int et = entityStream.readByte();
			ElementType elemType = ElementType.fromValue(et);
