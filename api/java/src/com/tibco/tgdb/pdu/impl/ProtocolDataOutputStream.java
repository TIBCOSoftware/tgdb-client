 ...
 * Created by: suresh
 * <p/>
 * SVN Id: $Id: ProtocolDataOutputStream.java 3631 2019-12-11 01:12:03Z ssubrama $
 */
...
public class ProtocolDataOutputStream extends OutputStream implements TGOutputStream {
    ...
    public void writeBytes(byte[] buf) throws IOException {
        ...
        writeInt(len);
        if (len < 16) { //The loop is fast when the count is less than 16
            for (int i = 0; i < len; i++) {
                int v = buf[i];
                writeByte(v);
            }
            ...
            System.arraycopy(buf, 0, this.buffer, this.count, len);
            count += len;
        }
    }
    ...
    public void writeChars(String str) throws IOException {
        ...
        if (str == null) return;
        byte[] buf = str.getBytes(Charset.forName("UTF8"));
    	writeBytes(buf);
...
//        }
    ...
    public void writeUTF(String str) throws IOException {
        ...
        if (str == null) return;
        byte[] buf = str.getBytes(Charset.forName("UTF8"));
//        if (buf.length == 0) throw new IOException("Cannot convert to UTF8");

        int len = buf.length;
        ensure(len+2);
        writeShort(len);  //We are writing short here - so maximum length is 32K only.
        if (len < 16) { //The loop is fast when the count is less than 16
            for (int i = 0; i < len; i++) {
                int v = buf[i];
                writeByte(v);
...
//        /* The underlying code is JAVA UTF (char representation) and is potentially not available in the
//        other programming language/platform */
...
//        buffer[start+1] = (byte)(len & 0xFF);
    ...
    public static void main(String[] args) throws Exception {
        ProtocolDataOutputStream os = new ProtocolDataOutputStream();
        os.writeBoolean(true);
        os.writeByte(10);
        os.writeChar('c');
        os.writeShort(13456);
        os.writeInt(1234567);
        os.writeLong(123456789123L);
        os.writeFloat(1234.56f);
        os.writeDouble(123456.789);
        os.writeUTF("TestPduOutputStream");
        String s = HexUtils.formatHex(os.buffer, os.count);
        System.out.printf("%s\n",s);
    }
