...
func MultiTxn0(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) tgdb.TGNode {
	...
	for i:=0; i<1000; i++ {
		// Edge (Brother)
		edge, err := gof.CreateEdgeWithDirection(j1, j2, tgdb.DirectionTypeDirected)
		if err != nil {
...
func MultiTxn1(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, basicNodeType tgdb.TGNodeType) tgdb.TGNode {
...
func MultiTxn1_1(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) tgdb.TGNode {
	...
	entity, err := conn.GetEntity(key, impl.DefaultQueryOption())

	node := entity.(*impl.Node)

	fmt.Println(">>>>>>> Returning MultiTxn1_1 w/ NO ERROR!!!<<<<<<<")
	return node
}

func MultiTxn1_2(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, basicNodeType tgdb.TGNodeType) {
...
func MultiTxn2(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, john tgdb.TGNode) {
...
func MultiTxn3(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, basicNodeType tgdb.TGNodeType) (tgdb.TGNode, tgdb.TGEdge) {
	...
	// Edge (Brother)
	brother, err := gof.CreateEdgeWithDirection(smith, kelly, tgdb.DirectionTypeDirected)
	if err != nil {
...
func MultiTxn3_1(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) {
...
func MultiTxn4(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, john, kelly tgdb.TGNode) tgdb.TGEdge {
	fmt.Println(">>>>>>> Entering MultiTxn4: Add an edge between 2 existing nodes - In case between john and kelly <<<<<<<")

	// Edge (Wife)
	wife, err := gof.CreateEdgeWithDirection(kelly, john, tgdb.DirectionTypeDirected)
	if err != nil {
...
func MultiTxn4_0(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) tgdb.TGNode {
...
func MultiTxn5(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, wife tgdb.TGEdge) {
...
func MultiTxn6(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, john tgdb.TGNode) {
...
func MultiTxn6_1(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, john tgdb.TGNode) {
...
func MultiTxn7(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, brother tgdb.TGEdge) {
...
func MultiTxn8(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) {
	...
	result, err := conn.ExecuteQueryWithFilter(startFilter, "", traverserFilter, endFilter, impl.DefaultQueryOption())
	if err != nil {
