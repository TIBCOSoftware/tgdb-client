...
var (
	...
	mESCMap             = make(map[string]tgdb.TGNode, 0)
	hESCMap             = make(map[string]tgdb.TGNode, 0)
...
func importNodes(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) {
...
func importHESCEdges(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) {
	...
	for lineNo, line := range lineMap {
		...
		if fromNode != nil && toNode != nil {
			// Edge
			edge, err := gof.CreateEdgeWithDirection(fromNode, toNode, tgdb.DirectionTypeBiDirectional)
			if err != nil {
...
func importMESCEdges(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory) {
