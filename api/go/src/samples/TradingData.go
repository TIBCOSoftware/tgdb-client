...
func importStockValues(conn tgdb.TGConnection, gof tgdb.TGGraphObjectFactory, priceDataStack *Stack) {
	...
	//currMonth := 0
	//currWeek := 0
	var /*currStkYearNode, currStkMonthNode, currStkWeekNode, */currStkDayNode tgdb.TGNode
	var /*prevStkYearNode, prevStkMonthNode, prevStkWeekNode, */prevStkDayNode tgdb.TGNode

	dayNodeList := make([]tgdb.TGNode, 0)
	...
	for {
		...
		if prevStkDayNode != nil {
			// Edge
			nextDayEdge, err := gof.CreateEdgeWithDirection(prevStkDayNode, currStkDayNode, tgdb.DirectionTypeBiDirectional)
			if err != nil {
